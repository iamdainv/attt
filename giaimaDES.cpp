...
void GenKey(unsigned int K1, unsigned int K2, unsigned int key1[16], unsigned int key2[16])
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		printf("\nkhoa thu %d la : %7X %7X", i + 1, KC1, KD1);
	}
}
...
unsigned int F(unsigned int L0, unsigned int R0, unsigned int key1, unsigned int key2)
{
	...
	ER01 = ER0(R0, 0, 24);
	ER02 = ER0(R0, 24, 48);
	printf("\nMo rong : ER01 = %X , ER02 = %X ", ER01, ER02);
	//Buoc 5. ph�p XOR, t�nh A = E[R0] + K1.
	unsigned int A1, A2;
	A1 = key1 ^ ER01;
	A2 = key2 ^ ER02;
	printf("\nXOR : A = E[R0] + K1 = %X %X", A1, A2);
...
int main()
{
	unsigned int K1 = 0xF35D5147; //4byte
